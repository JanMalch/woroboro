...
data class ReminderEntity(
    ...
    @ColumnInfo("remind_at") val remindAt: LocalTime,
    @ColumnInfo("repeat_every") val repeatEvery: Duration?,
    @ColumnInfo("repeat_until") val repeatUntil: LocalTime?,
    // if filterRoutineId is set, the other values don't matter (see asModel below)
    @ColumnInfo("filter_routine_id") val filterRoutineId: UUID?,
    @ColumnInfo("filter_only_favorites") val filterOnlyFavorites: Boolean,
    @ColumnInfo("filter_duration") val filterDuration: DurationFilter,
    @ColumnInfo("routines_order", defaultValue = "NameAsc") val routinesOrder: RoutinesOrder,
    @ColumnInfo("is_active", defaultValue = "1") val isActive: Boolean,
)
...
data class ReminderFilterTagCrossRefEntity(
    @ColumnInfo(name = "reminder_id", index = true) val reminderId: UUID,
    @ColumnInfo(name = "tag_label", index = true) val tagLabel: String,
)
...
fun ReminderEntityWithFilterTags.asModel() =
    Reminder(
        id = reminder.id,
        name = reminder.name,
        ...
        repeat =
            if (reminder.repeatEvery != null && reminder.repeatUntil != null) {
                Reminder.Repeat(
        ...
        query =
            if (reminder.filterRoutineId != null) {
