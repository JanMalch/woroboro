...
VALUES      (?,
            ...
            db.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS `index_routine_step_id` ON `routine_step` (`id`)"
            )
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS `index_routine_step_routine_id` ON `routine_step` (`routine_id`)"
            )
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS `index_routine_step_sort_index` ON `routine_step` (`sort_index`)"
            )
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS `index_routine_step_exercise_id` ON `routine_step` (`exercise_id`)"
            )
        }
    }
